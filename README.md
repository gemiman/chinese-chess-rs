...
五张图串起一次完整的对局：选模式 → 下棋 → 杀招出场 → 终局分析 → 看两个 AI 对下。

> 截图由 [`scripts/dev/shots.mjs`](scripts/dev/shots.mjs) 驱动**真实界面**生成（无头浏览器里跑一遍
> 完整流程再截）。所以界面一改，重跑一次就能更新，不会留下过时的图 —— 手工截的图没人会想起来重截。

**① 开局设置** —— 三种对局模式（双人同机 / 人机 / 机机）+ 限时 + 走子速度 + **语音播报**（开关 / 音量 / 语速）

![开局设置页](docs/images/01-setup.png)

**② 对局中** —— 棋盘上下分置两条钟，轮到的一方显示步时倒计时；钟上还带着自然限着的进度

![对局页](docs/images/02-play.png)

**③ 战法名出场** —— 走出杀招时「绝杀」牌匾出场；下方是这一手的评价、讲解与战法标签

![绝杀出场特效](docs/images/03-mate.png)

**④ 终局分析** —— 结果、统计（含开局名称与失误数）、逐手讲解；可复盘、可再跑深度分析

![终局分析页](docs/images/04-analysis.png)

**⑤ 机机对战** —— 两个 AI 连播对局，钟上标着各自档位与胜场；随时可暂停、单步或返回设置

![机机对战](docs/images/05-auto.png)
...
| 工具脚本 | 素材 / 原型 / 知识库 / 文档 / CI 依赖断言 / 前端与桌面端冒烟测试 / 语音生成与播报 / **演示截图** | 11 个 |
...
### 2026-09-25 · README 加功能演示截图

§1.1 放了五张图，串起一次完整对局：选模式 → 下棋 → 杀招出场 → 终局分析 → 看两个 AI 对下。

**截图是脚本跑出来的**，不是手工截的：[`scripts/dev/shots.mjs`](scripts/dev/shots.mjs) 在无头浏览器里
驱动真实界面走完这些流程再截。手工截的图**会过时** —— 界面一改，README 里那张就跟不上，
而且没人会想起来重截；写成脚本之后重跑一次就更新了。

两个细节值得记：
- 视口用 **1380×940**，与 `tauri.conf.json` 里桌面窗口的尺寸一致 —— 所以截出来的**就是桌面端的样子**。
- 图片放 `docs/images/`，**不能放 `assets/`**：那是 vite 的 publicDir，放进去会被打进安装包，
  几百 KB 白占体积、用户还看不到。

顺带把分析页那张截成了「走完 12 手真实开局再认输」——只走一手就终局的话，统计里
「总手数 1 手」和一行棋谱看着不像个真东西。

 ...
 * - `hotseat` —— 双人同机，两边都是人
 * - `engine`  —— 人机对战，玩家对引擎
 * - `auto`    —— 机机对战，两边都是引擎（用来观战）
 */
export type GameMode = 'hotseat' | 'engine' | 'auto'
 ...
 * 机机对战：两局之间的间隔（毫秒）。
 *
 * 留出 20 秒是为了「一局刚下完，眼睛还盯在终局上」的那段时间 ——
 * 立刻换一局会让人反应不过来刚才那局是怎么结束的。
 */
export const AUTO_GAP_MS = 20_000

/**
 * 机机对战：**两次落子之间的最小间隔**（毫秒）。
 *
 * # 为什么是「节奏下限」而不是「让它多想一会儿」
 *
 * 一开始想过用「每步至少思考 3 秒」来实现，实测发现**对弱档位完全无效**：
 * 入门档只想 2 层，给它 3000 毫秒的预算，它 **0 毫秒**就算完返回了 ——
 * 弱档位的设计就是「算完即走」，时间预算对它没有任何约束力。
 * （中级 169 毫秒、大师才真用满 3000 毫秒。）
 *
 * 所以想让棋别从眼前飞过去，只能在**走子节奏**上加下限：引擎算得再快，
 * 也等满这 3 秒再走下一步。好处是档位的含义一点没变 —— 入门还是入门。
 */
export const AUTO_PACE_MS = 3_000
